USE [PS_GameData]
GO
/****** Object:  Trigger [dbo].[Perfect_Orange_Stats_Add1]    Script Date: 01/21/2023 01:25:49 ******/
SET ANSI_NULLS ON
GO
    ...
    BEGIN
		...
		BEGIN
			...
			SET @NewEnchant = @Enchant
				...
				IF (@RecRuneItemID = 101031 AND @NewStr >=15 AND @NewStr < @ReqWis) -- STR +1
				BEGIN				
					SET @NewStr = @Str+1
				...
				ELSE IF (@RecRuneItemID = 101032 AND @NewDex >=15 AND @NewDex < @ReqWis) -- DEX +1
				BEGIN				
					SET @NewDex = @Dex+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101035 AND @NewRec >=15 AND @NewRec <= @ReqWis) -- REC +1
				BEGIN				
					SET @NewRec = @Rec+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101033 AND @NewInt >=15 AND @NewInt <= @ReqWis) -- INT +1
				BEGIN				
					SET @NewInt = @Int+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101034 AND @NewWis >=15 AND @NewWis <= @ReqWis) -- WIS +1
				BEGIN				
					SET @NewWis = @Wis+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101036 AND @NewLuc >=15 AND @NewLuc <= @ReqWis) -- LUC +1
				BEGIN				
					SET @NewLuc = @Luc+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101037 AND @NewHP >=15 AND @NewHP <= @ReqWis) -- HP +100
				BEGIN				
					SET @NewHP = @HP+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101038 AND @NewMP >=15 AND @NewMP <= @ReqWis) -- MP +100
				BEGIN				
					SET @NewMP = @MP+1
					UPDATE UserStoredItems
					SET Count -= 1
				...
				ELSE IF (@RecRuneItemID = 101039 AND @NewSP >=15 AND @NewSP <= @ReqWis) -- SP +100
				BEGIN				
					SET @NewSP = @SP+1
					UPDATE UserStoredItems
					SET Count -= 1
